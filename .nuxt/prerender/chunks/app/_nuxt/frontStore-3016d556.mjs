...
const useFrontStore = defineStore("frontStore", {
  state: () => ({
    loading: false,
    serverError: null,
    error: null,
    ...
    courses: [
      {
        ...
        description: 'Hi there, myself Munna Ahmed. I am fond of coding with javascript. as a javascript lover. after completing my graduation I was admitted to the admirable company "ThemeBracket" as an intern in 2018. after completing 3 months of the intern period I was proposed for playing the role of a frontend developer there. as a rising talent, my job career started then in the same year.'
      },
      {
        id: 2,
        thumb: "c2.png",
  ...
  actions: {
    ...
    getIdPost(id) {
      return this.posts.find((p) => {
        return p.title.trim().toLowerCase().replaceAll(/[^\w\s]/gi, "").replaceAll(" ", "-").replaceAll("--", "-") === id;
      });
    },
    getIdCourse(id) {
      return this.courses.find((c) => {
        return c.title.trim().toLowerCase().replaceAll(/[^\w\s]/gi, "").replaceAll(" ", "-").replaceAll("--", "-") === id;
      });
    },
    ...
    updateQuantity(id, quantity) {
      const item = this.cart.find((item2) => item2.id === id);
      if (item) {
  ...
  getters: {
    ...
    filteredSearchCourses() {
      return this.courses.filter((item) => {
        return item.title.toLowerCase().includes(this.searchCourse.toLowerCase()) && (!this.searchPrice || item.price <= this.searchPrice) && (!this.searchLang || this.searchLang == item.language.replace(/[\s\W]/g, "").toLowerCase()) && (item.skillLevel.toLowerCase() == this.searchSkill.toLowerCase() || !this.searchSkill) && (item.category.toLowerCase().trim() == this.searchCate || !this.searchCate) && (Math.floor(item.rating) >= this.searchRate && Math.floor(item.rating) < this.searchRate + 1 || !this.searchRate);
      });
    },
...
export { useFrontStore as u };
//# sourceMappingURL=frontStore-3016d556.mjs.map
